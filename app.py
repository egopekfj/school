...
def format_stats_record(
    student_data, test_title, score, max_score, grade, timestamp=None
):
  """Формирует словарь записи результатов ученика для статистики."""
  ...
  return {
      ...
      "test": test_title if test_title else "Без названия",
      "score": round(score, 2),
      "total": round(max_score, 2),
      "grade": grade,
...
def make_fullscreen(window):
  """Разворачивает окно и добавляет горячие клавиши F11 и Esc."""
  try:
    window.state("zoomed")
  except Exception:
    pass
...
def contains_any_correct_word(user_answer, accepted_answers):
  """Проверяет наличие хотя бы одного правильного слова."""
  user_norm = normalize_answer(user_answer)
  user_tokens = set(user_norm.split())
...
def score_text_question(q, user_answer):
  """Подсчёт баллов за письменный вопрос с поддержкой всех режимов."""
  ...
  full_points = max(0.0, full_points)
  partial_points = max(0.0, min(partial_points, full_points))
  ...
  if partial_points > 0:
    ...
    if mode in ["contains", "exact"] and contains_any_correct_word(
        user_answer, answers
    ):
      return partial_points
...
class TestApp:
  ...
  def setup_menu(self):
    ...
    tk.Label(
        self.root,
        text=(
            "Оффлайн • Таймер • Выбор ответа • Письменный ответ • Частичный"
            " балл • Статистика"
        ),
        fg="gray",
        font=("Segoe UI", 9),
...
class TeacherWindow(tk.Toplevel):
  ...
  def setup_ui(self):
    ...
    tk.Label(
        main,
        text=(
            "💡 При каждом запуске ученик получит случайные вопросы из вашего"
            " пула"
        ),
        fg="#666",
        font=("Segoe UI", 9),
    ...
    self.ent_correct_text = tk.Entry(
        self.text_container, font=("Segoe UI", 11)
    )
    ...
    tk.Label(
        self.text_container,
        text=(
            "Частичный балл начисляется, если ответ содержит правильный"
            " фрагмент/слово/букву, но не совпадает полностью. 0 — отключить."
        ),
        fg="#666",
        font=("Segoe UI", 8),
        wraplength=560,
    ...
    self.listbox = tk.Listbox(
        main, height=9, font=("Segoe UI", 10), activestyle="none"
    )
    ...
    if q_type == "choice":
      ...
      if correct < 0 or correct >= len(opts):
        messagebox.showwarning(
            "Внимание", "Номер правильного ответа больше количества вариантов"
        )
        return
      ...
      if not raw_answers:
        messagebox.showwarning(
            "Внимание", "Введите правильный ответ для письменного вопроса"
        )
        return
      ...
      if not answers:
        messagebox.showwarning("Внимание", "Введите хотя бы один правильный ответ")
        return
  ...
  def edit_question(self):
    if self.selected_idx is None:
      messagebox.showinfo(
          "Инфо", "Выберите вопрос из списка для редактирования"
      )
      return
  ...
  def refresh_list(self):
    ...
    for i, q in enumerate(self.test_data["questions"]):
      ...
      self.listbox.insert(
          "end", f"{i + 1}. {marker} {display_text} | ⏱ {time_q} с | {points} б."
      )
  ...
  def update_test_metadata(self):
    ...
    try:
      ...
      if take_val <= 0 or take_val > total_q:
        take_val = total_q
      self.test_data["take_questions"] = take_val
    except ValueError:
      self.test_data["take_questions"] = total_q

    if "grading" not in self.test_data or not isinstance(
        self.test_data["grading"], dict
    ):
      self.test_data["grading"] = {"5": 0, "4": 0, "3": 0, "2": 0}

    for g in ["5", "4", "3", "2"]:
      current = self.test_data["grading"].get(g, 0)
      self.test_data["grading"][g] = safe_float(
          self.grade_ents[g].get(), current
      )
  ...
  def load_test(self):
    ...
    try:
      ...
      self.ent_take.insert(
          ...
          str(
              self.test_data.get(
                  "take_questions", len(self.test_data.get("questions", []))
              )
          ),
      )
      ...
      for g in ["5", "4", "3", "2"]:
        ...
        self.grade_ents[g].insert(
            0, format_number(self.test_data.get("grading", {}).get(g, 0))
        )
...
class PreviewWindow(tk.Toplevel):
  ...
  def build_preview_text(self):
    ...
    lines = [
        "ПРЕДПРОСМОТР ТЕСТА",
        "=" * 45,
        f"Название теста: {data.get('title', 'Тест')}",
        f"Всего вопросов в пуле: {len(questions)}",
        (
            "Ученику будет выдано:"
            f" {data.get('take_questions', len(questions))}"
        ),
        "",
        "Критерии оценок:",
    ]
    ...
    for mark in ["5", "4", "3", "2"]:
      lines.append(
          f"   Оценка {mark}: от {format_number(grading.get(mark, 0))} баллов"
      )

    lines.extend(["", "ВОПРОСЫ:", "-" * 45, ""])
    ...
    for i, q in enumerate(questions, 1):
      ...
      if q_type == "text":
        ...
        lines.append(
            "   Правильные ответы:"
            f" {'; '.join(answers) if answers else 'не заданы'}"
        )
        lines.append(
        ...
        if 0 <= correct < len(options):
          lines.append(
              f"   Правильный ответ: {correct + 1}) {options[correct]}"
          )
        else:
  ...
  def save_json(self):
    path = filedialog.asksaveasfilename(
        ...
        filetypes=[("JSON", "*.json")],
        initialfile=(
            f"{self.test_data.get('title', 'Тест').replace(' ', '_')}.json"
        ),
    )
...
class StudentWindow(tk.Toplevel):
  ...
  def setup_ui(self):
    ...
    tk.Label(self.reg_frame, text="Фамилия:", font=("Segoe UI", 11, "bold")).pack(
        anchor="w"
    )
    ...
    self.lbl_prog = tk.Label(
        self.test_frame, text="", font=("Segoe UI", 10), fg="gray"
    )
    self.lbl_prog.pack()

    self.lbl_points = tk.Label(
        self.test_frame, text="", font=("Segoe UI", 9), fg="gray"
    )
  ...
  def load_test(self):
    ...
    try:
      ...
      take_n = safe_float(
          self.test.get("take_questions", len(questions)), len(questions)
      )
      take_n = int(take_n) if take_n > 0 else len(questions)
      ...
      messagebox.showerror(
          "Ошибка", f"Файл повреждён или неверный формат:\n{e}"
      )
  ...
  def show_q(self):
    ...
    self.lbl_prog.config(
        text=f"Вопрос {self.current + 1} из {len(self.questions)}"
    )
    ...
    if q_type == "text":
      ...
      if mode == "letter":
        ...
        hint = "Введите знак дефиса (-):"
      elif mode == "contains":
        hint = (
            "Введите ответ (правильный ответ может быть в составе вашего"
            " ответа):"
        )
      else:
        hint = "Введите ответ:"
      ...
      self.correct_idx = next(
          (i for i, (_, is_correct) in enumerate(pairs) if is_correct), -1
      )
  ...
  def update_timer(self):
    ...
    self.lbl_timer.config(fg="#e63946" if self.time_left <= 5 else "#2b2d42")
  ...
  def finish(self):
    ...
    test_title = (
        self.test.get("title", "Без названия") if self.test else "Без названия"
    )
    record = format_stats_record(
        self.student, test_title, self.score, total, grade
    )
    ...
    try:
      ...
      messagebox.showwarning(
          "Статистика", f"Не удалось сохранить статистику:\n{e}"
      )

    for w in self.winfo_children():
      w.destroy()

    tk.Label(self, text="✅ Тест завершён!", font=("Segoe UI", 20, "bold")).pack(
        pady=30
    )
    tk.Label(
        self,
...
class StatsWindow(tk.Toplevel):
  ...
  def setup_ui(self):
    ...
    tk.Label(
        top, text="Статистика прохождения тестов", font=("Segoe UI", 16, "bold")
    ).pack(anchor="w")

    ctrl = tk.Frame(top)
    ctrl.pack(fill="x", pady=10)

    tk.Label(ctrl, text="Ученик:", font=("Segoe UI", 10, "bold")).pack(
        side="left"
    )

    self.student_combo = ttk.Combobox(ctrl, state="readonly", width=55)
    ...
    tk.Button(ctrl, text="🔄 Обновить", command=self.refresh).pack(
        side="left", padx=5
    )
    ...
    tk.Label(
        bottom,
        text=(
            "Выберите строку в таблице, чтобы удалить одну или несколько"
            " записей."
        ),
        fg="gray",
  ...
  def make_student_display(self, rec):
    return (
        f"{rec.get('last_name', '')} {rec.get('first_name', '')} — группа"
        f" {rec.get('group', '')}".strip()
    )
  ...
  def delete_selected_record(self):
    ...
    if not selected:
      messagebox.showinfo("Информация", "Сначала выберите запись в таблице.")
      return

    if not messagebox.askyesno(
        "Подтверждение", f"Удалить выбранные записи ({len(selected)} шт.)?"
    ):
      return
  ...
  def delete_student_all(self):
    ...
    if not key:
      messagebox.showinfo(
          "Информация", "Выберите ученика в списке или в таблице."
      )
      return

    if not messagebox.askyesno(
        "Подтверждение", f"Удалить всю статистику для ученика:\n{display}?"
    ):
      return
